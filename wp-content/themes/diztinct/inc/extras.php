 ...
 */
function wds_proclubs_categorized_blog() {
	$category_count = get_transient( 'wds_proclubs_categories' );
	...
	if ( false === $category_count ) {
		...
		set_transient( 'wds_proclubs_categories', $category_count );
	}
 ...
 */
function wds_proclubs_get_attachment_id_from_url( $attachment_url = '' ) {
	global $wpdb;
 ...
 */
function wds_proclubs_copyright_year( $atts ) {
	// Setup defaults.
	$args = shortcode_atts(
		[
			'starting_year' => '',
...
add_shortcode( 'wds_proclubs_copyright_year', 'wds_proclubs_copyright_year', 15 );
