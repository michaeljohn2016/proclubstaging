 ...
 */
function wds_proclubs_posted_on() {
	...
	$posted_on = sprintf(
		...
		esc_html_x( 'Posted on %s', 'post date', 'proclubs' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		/* translators: the post author */
		esc_html_x( 'by %s', 'post author', 'proclubs' ),
 ...
 * @author WebDevStudios
 */
function wds_proclubs_entry_footer() {
	// Hide category and tag text for pages.
	if ( 'post' === get_post_type() ) {
		/* translators: used between list items, there is a space after the comma */
		$categories_list = get_the_category_list( esc_attr__( ', ', 'proclubs' ) );
		if ( $categories_list && wds_proclubs_categorized_blog() ) {

			/* translators: the post category */
			printf( '<span class="cat-links">' . esc_attr__( 'Posted in %1$s', 'proclubs' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_attr__( ', ', 'proclubs' ) );
		if ( $tags_list ) {

			/* translators: the post tags */
			printf( '<span class="tags-links">' . esc_attr__( 'Tagged %1$s', 'proclubs' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
		}
	}

	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( esc_attr__( 'Leave a comment', 'proclubs' ), esc_attr__( '1 Comment', 'proclubs' ), esc_attr__( '% Comments', 'proclubs' ) );
		echo '</span>';
	}
 ...
 */
function wds_proclubs_display_svg( $args = [] ) {
 ...
 */
function wds_proclubs_get_svg( $args = [] ) {
	// Make sure $args are an array.
	if ( empty( $args ) ) {
		return esc_attr__( 'Please define default parameters in the form of an array.', 'proclubs' );
	}

	// Define an icon.
	if ( false === array_key_exists( 'icon', $args ) ) {
		return esc_attr__( 'Please define an SVG icon filename.', 'proclubs' );
	}
	...
	<svg
	<?php
		echo wds_proclubs_get_the_content( $height ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
		echo wds_proclubs_get_the_content( $width ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
		echo wds_proclubs_get_the_content( $color ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
		echo wds_proclubs_get_the_content( $stroke_width ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
	?>
		class="icon <?php echo esc_attr( $args['icon'] ); ?>"
	<?php
		echo wds_proclubs_get_the_content( $aria_hidden ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
		echo wds_proclubs_get_the_content( $aria_labelledby ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
	?>
		role="img">
 ...
 */
function wds_proclubs_get_the_title( $args = [] ) {
	// Set defaults.
	$defaults = [
		'length' => 12,
		'more'   => '...',
 ...
 */
function wds_proclubs_get_the_excerpt( $args = [] ) {
 ...
 * @author WebDevStudios
 */
function wds_proclubs_display_copyright_text() {
	// Grab our customizer settings.
	$copyright_text = get_theme_mod( 'wds_proclubs_copyright_text' );

	if ( $copyright_text ) {
		echo wds_proclubs_get_the_content( do_shortcode( $copyright_text ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
	}
}
 ...
 * @author WebDevStudios
 */
function wds_proclubs_display_social_network_links() {
	...
	<ul class="flex social-icons menu">
		...
		foreach ( $social_networks as $network ) :
			...
			$network_url = get_theme_mod( 'wds_proclubs_' . $network . '_link' );
 ...
 */
function wds_proclubs_display_numeric_pagination( $args = [], $query = null ) {
	if ( ! $query ) {
		global $wp_query;
		$query = $wp_query;
	}
	...
	<nav class="container pagination-container" aria-label="<?php esc_attr_e( 'numeric pagination', 'proclubs' ); ?>">
 ...
 */
function wds_proclubs_display_mobile_menu() {
	...
	<nav class="off-canvas-container" aria-label="<?php esc_attr_e( 'Mobile Menu', 'proclubs' ); ?>" aria-hidden="true" tabindex="-1">
		<?php
		// Mobile menu args.
		$mobile_args = [
 ...
 * @author WebDevStudios
 */
function wds_proclubs_display_comments() {
