 ...
 */
function wds_proclubs_body_classes( $classes ) {
...
add_filter( 'body_class', 'wds_proclubs_body_classes' );

/**
 * Flush out the transients used in wds_proclubs_categorized_blog.
 *
 * @author WebDevStudios
 *
 * @return bool Whether or not transients were deleted.
 */
function wds_proclubs_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return false;
	}

	// Like, beat it. Dig?
	return delete_transient( 'wds_proclubs_categories' );
}

add_action( 'delete_category', 'wds_proclubs_category_transient_flusher' );
add_action( 'save_post', 'wds_proclubs_category_transient_flusher' );
 ...
 */
function wds_proclubs_content_more_link() {
	return ' <a class="more-link" href="' . get_permalink() . '">' . esc_html__( 'Read More', 'proclubs' ) . '...</a>';
}

add_filter( 'the_content_more_link', 'wds_proclubs_content_more_link' );
 ...
 * @return string Read more link.
 */
function wds_proclubs_excerpt_more( $more ) {
	return sprintf( ' <a class="more-link" href="%1$s">%2$s</a>', get_permalink( get_the_ID() ), esc_html__( 'Read more...', 'proclubs' ) );
}

add_filter( 'excerpt_more', 'wds_proclubs_excerpt_more' );
 ...
 */
function wds_proclubs_get_the_content( $content ) {
	return ! empty( $content ) ? $content : false;
}

add_filter( 'the_content', 'wds_proclubs_get_the_content', 20 );
 ...
 */
function wds_proclubs_custom_mime_types( $mimes ) {
...
add_filter( 'upload_mimes', 'wds_proclubs_custom_mime_types' );
 ...
 * @author WebDevStudios
 */
function wds_proclubs_include_svg_icons() {
...
add_action( 'wp_footer', 'wds_proclubs_include_svg_icons', 9999 );
 ...
 */
function wds_proclubs_display_customizer_header_scripts() {
	// Check for header scripts.
	$scripts = get_theme_mod( 'wds_proclubs_header_scripts' );
	...
	echo wds_proclubs_get_the_content( $scripts );
}

add_action( 'wp_head', 'wds_proclubs_display_customizer_header_scripts', 999 );
 ...
 */
function wds_proclubs_display_customizer_footer_scripts() {
	// Check for footer scripts.
	$scripts = get_theme_mod( 'wds_proclubs_footer_scripts' );
...
add_action( 'wp_footer', 'wds_proclubs_display_customizer_footer_scripts', 999 );
 ...
 */
function wds_proclubs_add_og_tags() {
	...
	$default_base_description = ( get_bloginfo( 'description' ) ) ? get_bloginfo( 'description' ) : esc_html__( 'Visit our website to learn more.', 'proclubs' );
	...
	if ( is_category() || is_tag() || is_tax() ) {
		...
		$specify        = ( is_category() ) ? esc_html__( 'categorized in', 'proclubs' ) : esc_html__( 'tagged with', 'proclubs' );
		$queried_object = get_queried_object();
		$card_url       = get_term_link( $queried_object );
		$card_type      = 'website';

		// Translators: get the term name.
		$card_long_description = sprintf( esc_html__( 'Posts %1$s %2$s.', 'proclubs' ), $specify, $term_name );
	...
	if ( is_search() ) {
		...
		$card_long_description = sprintf( esc_html__( 'Search results for %s.', 'proclubs' ), $search_term );
		$card_description      = $card_long_description;
	}
...
add_action( 'wp_head', 'wds_proclubs_add_og_tags' );
 ...
 */
function wds_proclubs_remove_archive_title_prefix( $block_title ) {
...
add_filter( 'get_the_archive_title', 'wds_proclubs_remove_archive_title_prefix' );
 ...
 * @author Corey Collins
 */
function wds_proclubs_disable_wpautop_for_gutenberg() {
...
add_filter( 'init', 'wds_proclubs_disable_wpautop_for_gutenberg', 9 );
